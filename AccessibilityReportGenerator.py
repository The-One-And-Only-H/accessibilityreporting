...
def chooseTask(args):
    '''Run either Axe or Lighthouse from the command line'''
    if args.package == 'axe':
        return AxeTask()
    elif args.package == 'lighthouse':
        return LighthouseTask()
    elif args.package == 'metatags':
        return MetaTagsTask()

def aggregateResults(results):
    '''Filter through problems flagged by the report and collate duplicates'''
